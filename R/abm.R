...
#' initialise_agents
#'
#' sets the initial state variables at the beginning of each run (Jan 2019 before smart-meter rollout)
#'
...
initialise_agents <- function(sD, start_year=2019){
  ...
  #
  agents_in <- dep_survey %>% dplyr::select(serial,q14,q15,q41,Q41_oth,qi)
  ...
  agents_in <- agents_in %>% dplyr::select(serial,kWh,tariff)
  agents_in %>% return()
}
